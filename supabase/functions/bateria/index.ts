...
body{background:#000;color:white;min-height:100vh;display:flex;flex-direction:column;font-family:system-ui,sans-serif}
...
<main>
  <div class="b"><div id="f-container" class="f-container"><div class="empty">Aún no tienes posiciones<br><small>Paga para empezar</small></div></div></div>
...
const supabase = createClient(
  Deno.env.get('SUPABASE_URL') || 'https://vfnouwdawgkrtexudfkj.supabase.co',
...
async function init() {
  ...
  user = u
  userInfo.textContent = user.user_metadata?.full_name || user.email.split('@')[0]

  const channel = supabase.channel(\`bateria-\${user.id}\`)
  channel.on('postgres_changes',{
    event:'*',
    schema:'public',
    table:'bateria_posiciones',
...
async function renderBateria() {
  if (!user) return
  const { data, error } = await supabase.from('bateria_posiciones')
    .select('posicion').eq('user_id', user.id).order('posicion',{ascending:true})
  if (error) return console.error(error)

  container.innerHTML = data.length===0 ? '<div class="empty">Aún no tienes posiciones<br><small>Paga para empezar</small></div>' : ''
  data.forEach((row,i)=>{
    const bar = document.createElement('div')
    bar.className = 'f-bar'
    bar.textContent = \`\${row.posicion}%\`
...
window.confirmarPago = async ()=>{
  const monto = Number(document.getElementById('monto-input').value)
  if(isNaN(monto)||monto<1) return alert('Monto inválido')
