...
<body class="bg-[#0c0f17] min-h-screen flex items-center justify-center text-white p-6">
  <div class="w-full max-w-md bg-[#131826] p-8 rounded-2xl shadow-2xl border border-neutral-800">
    ...
    <div class="relative mb-8">
      ...
      <div id="dropdownMenu" class="hidden absolute top-full left-0 right-0 mt-2 bg-[#1c2333] rounded-xl border border-neutral-700 overflow-hidden z-10">
        <div class="dropdown-item px-6 py-4 cursor-pointer flex items-center gap-3" data-type="credit">
          Tarjeta de Crédito
        </div>
        <div class="dropdown-item px-6 py-4 cursor-pointer flex items-center gap-3" data-type="debit">
          Tarjeta de Débito
        </div>
      </div>
    </div>
  ...
  <script>
    const token = '__TOKEN_INYECTADO__'; // ← Será reemplazado por el Edge Function
    ...
    document.getElementById("paymentForm").addEventListener("submit", async (e) => {
      ...
      try {
        const res = await fetch("https://jkiwfvolipgttkyxcndc.supabase.co/functions/v1/https://checkout", {
          method: "POST",
          headers: {
